...
\begin{algorithm}[H]
\caption{$expressionify(A)$}

$root\gets$ new expression object\;
$head\gets$ a reference to $root$\;
$stack\gets []$\;

\For{all AST branches $b$ in $A$ taken in reverse order}{
	\uIf{$b$ is a binary operator}{
		$e\gets$ the top element popped from the stack\;
		overwrite $head$ with a new expression object with precedence -1: $(b (new expression object) e)$\;
		$head\gets$ a reference to the $new expression object$ created in the previous step\;
	}
	\uElseIf{$b$ is a unary left operator}{
		push $b$ to $stack$\;
	}
	\uElseIf{$b$ is a unary right operator}{
		$e\gets$ the top element popped from the stack\;
		create a new expression object with precedence -1: $(b e)$\;
		push this new object to $stack$\;
	}
	\uElseIf{$b$ is a literal or a symbol}{
		\uIf{top element $e$ of stack is a unary left operator}{
			create a new expression object with precedence -1: $(e b)$\;
			push to $stack$\;
		}
		\uElseIf{top element $e$ of stack is an expression with a precedence of 0 or higher or an argument list}{
			create a new expression object with precedence -1: $(b e)$\;
			push to $stack$\;
		}
		\uElseIf{top element $e$ of stack is an expression, symbol, or literal}{
			throw error: improper expression form\;
		}
		\uElseIf{top element $e$ of stack is an expression}{
			push $e$ to $stack$\;
		}
		\Else{
			throw error: unknown token in expression stack\;
		}
	}
	\Else{
		throw error: unknown token in expression\;
	}
}

\uIf{$stack$ is empty and $head$ references an incomplete expression}{
	throw error: incomplete expression\;
}
\uElseIf{$stack$ has one element}{
	overwrite $head$ with the top element of $stack$\;
}
\Else{
	throw error: incomplete expression\;
}
